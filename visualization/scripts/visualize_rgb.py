...
from visualization.src.rgb_visualization import (
    DEFAULT_CAMERA,
    DEFAULT_DATA_ROOT,
    DEFAULT_FRAME,
    DEFAULT_OUTPUT_ROOT,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the RGB image for the fixed AnyGraspNet sample frame."
    )
    parser.add_argument(
        "--data-root",
...
def main() -> None:
    ...
    output_path = export_rgb_image(paths.rgb_path, paths.output_rgb_path)
    print(f"RGB source: {paths.rgb_path}")
    print(f"RGB output: {output_path}")
